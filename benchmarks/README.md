...
| `ocean_warm_bubble` | `moist_compressible_XZ` (mc only) | 300 × 25 km, 2 h, warm bubble over a fixed-SST ocean (302.65 K), Louis BL + bulk surface fluxes + Komori drag ON, or the MYNN-EDMF arm (`SCYTHE_OWB_BL=mynn`, prognostic `rho_e` TKE, `_MYNN_INTERVAL` / `_MYNN_KMAX`); `SCYTHE_OWB_*` knobs for ice / radiation / BL arms (ice + MYNN is the supported pair, S8; ice + Louis is refused by the driver) | 3 km × 300 m / 6 km × 500 m (the TC nests' spacings) | none — control-run windows only (MYNN-EDMF plan S1; RCE precursor) |
